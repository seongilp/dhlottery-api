...
def buy_lotto645(
    ...
    exclude_last_draw: Annotated[bool, typer.Option("-x", "--exclude-last-draw", help="직전 회차 당첨번호(보너스 포함)를 제외한 랜덤 번호로 수동 5장을 구매합니다.")] = False,
    profile: Annotated[str, typer.Option("-p", "--profile", help="프로필을 지정합니다", metavar="")] = "default",
    _debug: Annotated[bool, typer.Option("-d", "--debug", help="debug 로그를 활성화합니다.", callback=logger_callback)] = False,
):
    if exclude_last_draw and tickets:
        print("❌ --exclude-last-draw 옵션은 번호를 직접 지정할 때는 사용할 수 없습니다.")
        raise typer.Exit(code=1)
    ...
    if exclude_last_draw:
        previous_round, winning_numbers = client.get_previous_lotto645_winning_numbers()
        print(f"ℹ️ 직전 회차({previous_round}회) 당첨번호를 제외하고 번호를 생성합니다: {' '.join(map(str, winning_numbers[:6]))} + 보너스 {winning_numbers[6]}")
        tickets = Lotto645Ticket.create_tickets_excluding(winning_numbers, count=5)
    else:
